...
UpPyramid::UpPyramid(int n,double *foundation,double *staging,int substeps,int dataPointSize, int constants,int conservedQuantities)
{
	...
	this->northPanel = new double[panelSize + (panelSize * constants)];
	this->southPanel = new double[panelSize + (panelSize * constants)];
	this->eastPanel  = new double[panelSize + (panelSize * constants)];
	this->westPanel  = new double[panelSize + (panelSize * constants)];	
}

int UpPyramid::ijToIndex(int i,int j)
{
	return Swept2DUtils::ijToIndex(n,i,j,this->substeps,this->dataPointSize,this->constants);
}
...
{
	int indexStart = Swept2DUtils::getShrinkingIndex(this->n,level,this->substeps,this->dataPointSize,this->constants);
	...
	for(int i=level+1;i<level+2+1;i++)
	{
		for(int j=0+level+1;j<(n-2)-level+1;j++)
		{
			...
			memcpy(&this->northPanel[index],&sourceData[this->ijToIndex(i,j)],((substeps * dataPointSize) + constants) * sizeof(double));			
	...
	for(int i=(n-2)-level-2+1;i<(n-2)-level+1;i++)
	{
		for(int j=0+level+1;j<(n-2)-level+1;j++)
		{
			memcpy(&this->southPanel[index],&sourceData[this->ijToIndex(i,j)],((substeps * dataPointSize) + constants) * sizeof(double));
			index += (substeps * dataPointSize) + constants;	
		}
	}
	...
	for(int j=(n-2)-level-2+1;j<(n-2)-level+1;j++)
	{
		...
		{
			memcpy(&this->eastPanel[index],&sourceData[this->ijToIndex(i,j)],((substeps * dataPointSize) + constants) * sizeof(double));
			index += (substeps * dataPointSize) + constants;			
		}
	}
	...
	for(int j=level+1;j<level+2+1;j++)
	{
		for(int i=0+level+1;i<(n-2)-level+1;i++)		
		{
			memcpy(&this->westPanel[index],&sourceData[this->ijToIndex(i,j)],((substeps * dataPointSize) + constants) * sizeof(double));
			index += (substeps * dataPointSize) + constants;			
		}
	}
