...
void VerticalBridge::populateFoundation(int level,double *destData)
{
	...
	for(int j=(n-2)/2-(2+level)+1;j<(n-2)/2-level+1;j++)
	{
		...
		{
			memcpy(&destData[this->ijToIndex(j,i)],&this->northPanel[index],((substeps * dataPointSize) + constants) * sizeof(double));			
	...
	for(int j=(n-2)/2+(level)+1;j<(n-2)/2+2+level+1;j++)
	{
		for(int i=level+1;i<(n-2)-level+1;i++)
		{
			memcpy(&destData[this->ijToIndex(j,i)],&this->southPanel[index],((substeps * dataPointSize) + constants) * sizeof(double));			
			index += (substeps * dataPointSize) + constants;
		}
	}
...
void VerticalBridge::buildPanels(int level,double *sourceData)
{
	...
	for(int i=level+1;i<level+2+1;i++)
	{
		...
		{
			memcpy(&this->westPanel[index],&sourceData[this->ijToIndex(j,i)],((substeps * dataPointSize) + constants) * sizeof(double));			
			index += (substeps * dataPointSize) + constants;
		}
	}
	...
	for(int i=(n-2)-level-2+1;i<(n-2)-level+1;i++)
	{
		for(int j=(n-2)/2-(1+level)-1+1;j<((n-2)/2-(1+level))+((level+1)*2)-1+1;j++)
		{
			memcpy(&this->eastPanel[index],&sourceData[this->ijToIndex(j,i)],((substeps * dataPointSize) + constants) * sizeof(double));			
			index += (substeps * dataPointSize) + constants;
		}
	}	
}
