...
void HorizontalBridge::populateFoundation(int level,double *destData)
{
	...
	for(int j=(n-2)/2-(2+level)+1;j<(n-2)/2-level+1;j++)
	{
		...
		{
			memcpy(&destData[this->ijToIndex(i,j)],&this->eastPanel[index],((substeps * dataPointSize) + constants) * sizeof(double));
	...
	for(int j=(n-2)/2+(level)+1;j<(n-2)/2+2+level+1;j++)
	{
		for(int i=level+1;i<(n-2)-level+1;i++)
		{
			memcpy(&destData[this->ijToIndex(i,j)],&this->westPanel[index],((substeps * dataPointSize) + constants) * sizeof(double));
			index += (substeps * dataPointSize) + constants;			
		}
	}		
}
