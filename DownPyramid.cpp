...
DownPyramid::DownPyramid(int n,double *foundation,double *staging,int substeps,int dataPointSize,int constants,int conservedQuantities)
{
	...
	for(int i=0;i<(n+2) * (n+2) * substeps * dataPointSize + ((n+2) * (n+2) * constants);i++)
	{
		this->foundation[i] = 0;
		this->staging[i] = 0;
	}	
...
{
	int indexStart = Swept2DUtils::getGrowingIndex(this->n,level,this->substeps,this->dataPointSize,this->constants);
	...
	for(int j=(n-2)/2-level-1;j<(n-2)/2-level-1+2;j++)
	{
		...
		{
			memcpy(&sourceData[this->ijToIndex(i,j)],&this->westPanel[index],((substeps * dataPointSize) + constants) * sizeof(double));
	...
	{		
		...
		{
			memcpy(&sourceData[this->ijToIndex(i,j)],&this->northPanel[index],((substeps * dataPointSize) + constants) * sizeof(double));			
			index += (substeps * dataPointSize) + constants;
		}
	}
	...
	for(int i=(n-2)/2+level+1;i<(n-2)/2+level+1+2;i++)	
	{
		...
		{
			memcpy(&sourceData[this->ijToIndex(i,j)],&this->southPanel[index],((substeps * dataPointSize) + constants) * sizeof(double));
			index += (substeps * dataPointSize) + constants;
		}
	}
	...
	for(int j=(n-2)/2+level+1;j<(n-2)/2+level+1+2;j++)
	{
		for(int i=(n-2)/2-level+1;i<(n-2)/2-level+1+((level+1)*2);i++)
		{
			memcpy(&sourceData[this->ijToIndex(i,j)],&this->eastPanel[index],((substeps * dataPointSize) + constants) * sizeof(double));
			index += (substeps * dataPointSize) + constants;
		}
	}	
}
...
void DownPyramid::build(int executeFnc)
{
	...
	if(this->foundation != destData)memcpy(this->foundation,destData,sizeof(double) * ((n*n) * substeps * dataPointSize + ((n*n) * constants)));	
}
